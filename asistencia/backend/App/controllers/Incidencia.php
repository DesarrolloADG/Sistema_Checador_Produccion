...
html;
    ...
    public function incidenciaAdd(){
        $datos = new \stdClass();
        $economico = new \stdClass();
        $datos->catalogo_colaboradores_id = MasterDom::getData('colaborador_id');
        $economico->catalogo_colaboradores_id = MasterDom::getData('colaborador_id');
        $datos->fecha = MasterDom::getData('fecha');
        $economico->fecha_solicitud = '0000-00-00';
        $economico->fecha_dia_economico = MasterDom::getData('fecha');
        $economico->estatus = 1;
        $datos->catalogo_incidencia_id = MasterDom::getData('incidencia_id');
        $catalogo_incidencia = MasterDom::getData('incidencia_id');
        $datos->comentario = MasterDom::getData('comentario');

        $fecha = MasterDom::getData('fecha');
        $fechaEntera = strtotime($fecha);
        $mes = date("m", $fechaEntera);
        $economico->mes_dia_economico = $mes;
      ...
      if(MasterDom::getData('rango_fechas') == 'on'){
        ...
        while($fecha_inicio <= $fecha_fin){
            ...
            if($existe>0){
                ...
                //if($catalogo_incidencia == 18){
                //    IncidenciaDao::deletediaEconomico($economico);
                   // $ide = IncidenciaDao::insertDiaEconomico($economico);
                //}
            }else{
                $id = IncidenciaDao::insertProrrateoColaboradorIncidencia($datos);
              //  if($catalogo_incidencia == 18){
