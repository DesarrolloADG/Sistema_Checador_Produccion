...
class ResumenSemanal extends Controller{
  ...
  public function guardarPeriodoAsistenciaTest(){
    $periodo = ResumenSemanalDao::getPeriodoById(MasterDom::getData('periodo_id')); //recupera el valor de la variable enviada por post de jquery y realiza una consulta a la tabla prorrateo_periodo, obtiene un arreglo de resultados
    $nomnoi = MasterDom::getData('nomnoi'); //recupera el valor de la variable enviada por post de jquery
    $fecha_fin = new \DateTime($periodo['fecha_fin']); // da formato a la fecha recuperada del arreglo almacenado en $periodo

    $datos = new \stdClass(); //inicializa una clase generia creada por php
    $datos->tipo = ucwords(strtolower($periodo['tipo'])); //cambia la primera letra en mayusculas y el resto en minusculas de la variable $periodo['tipo']
    $administrador = ResumenSemanalDao::getDatosUsuarioLogeado(MasterDom::getSesion('usuario')); //realiza una consulta a la variable global $_SESSION y recupera el usuario logueado, hace una consulta para verificar el usuaro logueado
    ...
    $colaboradores = ResumenSemanalDao::getAllColaboradoresPago(ucwords(strtolower($periodo['tipo'])),$nomnoi, ''); //genera la consulta a la table colaboradores
          ...
          if( $num_semana == 2 || $catalogo_horario_id == 0){
            if($catalogo_horario_id == 0){
              ...
              for($llave=0; $llave<count($horarios); $llave++) {
                ...
                if($catalogo_horario_id != $catalogo_horario_id_anterior){
                  break;
                }
...
html;
    ...
    public function guardarPeriodo(){
      $periodo = ResumenSemanalDao::getPeriodoById(MasterDom::getData('periodo_id'));
      $pid = MasterDom::getData('periodo_id');
      $nomnoi = MasterDom::getData('nomnoi');
      $fecha_fin = new \DateTime($periodo['fecha_fin']);
      $dias_traductor = array(
          'Monday' => 'Lunes',
          'Tuesday' => 'Martes',
      ...
      $colaboradores = ResumenSemanalDao::getAllColaboradoresPago(ucwords(strtolower($periodo['tipo'])),$nomnoi, '');
      ...
      */
      ...
      // MRR
      $num = 0;
      $nominas = ResumenSemanalDao::consultacierresnomnoi($pid);      
      ...
      if($num == 4 || $nomnoi == ''){
        ...
        if(MasterDom::getData('tipo_periodo') == 'quincenal'){
          ResumenSemanalDao::updatePeriodo(MasterDom::getData('periodo_id'), MasterDom::getData('tipo_periodo'),'');          
        }
        else{
          ResumenSemanalDao::updatePeriodo(MasterDom::getData('periodo_id'), MasterDom::getData('tipo_periodo'), 3);  
        }        
      }
